...
// User-provided animation/config variables
...
const BUILD_LOOP = (CARDS, SPACING) => {
  ...
  // Keep scale at 1 so reflections (children) aren't squashed to 0.
  gsap.set(CARDS, { xPercent: 5000, opacity: 0, scale: 1 })
...
// Allow clicking a visible card on the left/right to navigate
// If the clicked card's center is to the right of the gallery center -> next, otherwise prev
CARDS.forEach(card => {
  // make it obvious clickable
  card.style.cursor = 'pointer'
  card.addEventListener('click', (e) => {
    // don't interfere with clicks on controls
    if (e.target.closest('.gallery__prev') || e.target.closest('.gallery__next')) return
    if (!GALLERY) return
    const galleryRect = GALLERY.getBoundingClientRect()
    const cardRect = card.getBoundingClientRect()
    const cardCenter = cardRect.left + cardRect.width / 2
    const galleryCenter = galleryRect.left + galleryRect.width / 2
    const delta = cardCenter - galleryCenter
    // small tolerance: if clicked near center, ignore
    if (Math.abs(delta) < Math.min(20, cardRect.width * 0.15)) return
    if (delta > 0) {
      SCRUB_TO(SCRUB.vars.totalTime + SPACING)
    } else {
      SCRUB_TO(SCRUB.vars.totalTime - SPACING)
